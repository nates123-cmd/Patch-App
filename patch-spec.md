...
Each suite app has an accent used for its pills/tags: course, stock, ink, tide, tick, break, today, crate, cue, patch, courseplus, resin (resin = an external app, not part of the suite — tracked here for bug capture only). The three item types also carry a tint — **bug** = accent yellow, **feature** = tide blue, **idea** = tick lavender.
...
The legacy single-text column `text` is retained nullable for back-compat (droppable once the backfill is confirmed). Apps: `course / stock / ink / tide / tick / break / today / crate / cue / patch / resin / courseplus`.
